...
function Door:init(x, y, height, direction, emitEvent)
    local endY = y + (direction == "up" and -height or height)
    ...
    self.nineSlice = gfx.nineSlice.new("assets/images/spritesheets/door.png", 2, 2, 16, 16)
...
function Door:draw(x, y, width, height)
    ...
    if self.dir == "up" then
        ...
        self.sliceRect.y = self.height - visibleHeight
    else
        local visibleHeight = self.height * (1 - progress)
        self.sliceRect.height = visibleHeight
        self.sliceRect.y = 0
